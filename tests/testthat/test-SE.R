...
test_that("SMD.SE from SMD and sample size", {
  ...
  expect_equal(
    dat %>%
      dplyr::mutate(SE = SE.SMD_from_SMD(SMD, n1, n2, method)) %>%
      dplyr::pull(SE),
...
test_that("Pooled standard error from CI", {
  ...
  expect_equal(suppressWarnings(dplyr::mutate(dat, SE = SEp_from_CIp(CI_low, CI_up, N1, N2, t_dist = t_dist))) %>% dplyr::pull(SE),
